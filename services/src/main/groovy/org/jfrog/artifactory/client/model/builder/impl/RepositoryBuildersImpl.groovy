...
class RepositoryBuildersImpl implements RepositoryBuilders {
    ...
    RemoteRepositoryBuilder builderFrom(RemoteRepository from) {
        new RemoteRepositoryBuilderImpl().packageType(from.packageType).blackedOut(from.blackedOut).description(from.description).excludesPattern(from.excludesPattern).handleReleases(from.handleReleases)
                ...
                .offline(from.offline).storeArtifactsLocally(from.storeArtifactsLocally).socketTimeoutMillis(from.socketTimeoutMillis).allowLenientHostAuthentication(from.allowLenientHostAuthentication)
                .cookieManagementEnabled(from.cookieManagementEnabled).localAddress(from.localAddress)
